...
- Author package entrypoint is `@renx-ai/plugin/tui`.
...
```json
{
  "$schema": "https://renx.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/renx-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@renx-ai/plugin/tui`.
- `@renx-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@renx-ai/plugin/tui"
...
- The value is a semver range checked against the running Renx version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.renx` is absent, no check is performed (backward compatible).
...
- `renx plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `renx plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.renx` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.renx`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.renx`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.renx` dir or `RENX_CONFIG_DIR`, Renx installs `@renx-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@renx-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Renx's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `resolveBindingSections` from `@renx-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.renx/themes` area near the plugin config source.
...
- `--pure` / `RENX_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.renx/plugins/tui-smoke.tsx`
- Local vim plugin: `.renx/plugins/tui-vim.tsx`
- Local smoke config: `.renx/tui.json`
- Local smoke theme: `.renx/plugins/smoke-theme.json`
