...
Practical reference for extracting a future `packages/server` from the current `packages/renx` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all renx services, Effect-first source of truth
- `packages/server` - renx server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full renx server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/renx`.
...
- those slices are mounted into the Hono server behind `RENX_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `renx` package split is earlier in the migration.
...
- the current server host still lives in `packages/renx`
...
- if `packages/server` imports services from `packages/renx` while `packages/renx` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/renx` as the temporary Hono host
...
- `packages/server` must not import from `packages/renx`

Allowed in phase 1:

- `packages/renx` imports `packages/server`
...
- `packages/renx` shrinks or disappears as package responsibilities are fully split
...
- `packages/renx` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/renx`
...
- host mounting from `packages/renx`
...
- no imports from `packages/renx` inside `packages/server`
...
- `packages/server` must still not import from `packages/renx`
...
### PR 4. Mount `packages/server` question from `packages/renx`

Scope:

- replace local experimental question route wiring in `packages/renx`
...
- mount it in parallel from `packages/renx`
...
- move adapter selection and server startup out of `packages/renx`

Rules:

- do not start this while `packages/server` still depends on `packages/renx`
...
- host logic no longer lives in `packages/renx`
...
3. the route is mounted from the current host in `packages/renx`
...
- `packages/server` imports services or runtime from `packages/renx`
- `packages/renx` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/renx` imports
...
- [ ] mount `question` from `packages/renx`
...
2. keep `packages/renx` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/renx`, stop and restructure the boundary first.
