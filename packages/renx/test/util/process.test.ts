...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.RENX_TEST ?? "")'), {
      env: {
        RENX_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "RENX_TEST_SHELL"], {
      shell: true,
      env: {
        RENX_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("RENX_TEST_SHELL=ok")
  })
