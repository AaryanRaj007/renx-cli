...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "renx.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "renx.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "renx.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "renx.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["renx.json", "renx.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "renx.json"),
        path.join(tmp.path, "renx.jsonc"),
        path.join(project, "renx.json"),
        path.join(project, "renx.jsonc"),
      ])
    })
  })
