...
import { EffectFlock } from "@renx/core/util/effect-flock"
...
import { AppFileSystem } from "@renx/core/filesystem"
import { Env } from "../../src/env"
import { provideTestInstance, provideTmpdirInstance } from "../fixture/fixture"
import { tmpdir } from "../fixture/fixture"
import { InstanceRuntime } from "@/project/instance-runtime"
import { CrossSpawnSpawner } from "@renx/core/cross-spawn-spawner"
...
import { Global } from "@renx/core/global"
...
const managedConfigDir = process.env.RENX_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "renx.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "renx.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://renx.ai/config.json",
      snapshot: false,
    })
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "renx.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "renx.jsonc")
...
test("ignores legacy tui keys in renx config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://renx.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "renx.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["RENX_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { renx: { options: { apiKey: "{env:RENX_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["renx"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["RENX_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["RENX_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "renx.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .renx directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const renxDir = path.join(dir, ".renx")
      await fs.mkdir(renxDir, { recursive: true })
      const agentDir = path.join(renxDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".renx", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .renx/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const renxDir = path.join(dir, ".renx")
      await fs.mkdir(renxDir, { recursive: true })

      const agentsDir = path.join(renxDir, "agents")
...
test("loads commands from .renx/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const renxDir = path.join(dir, ".renx")
      await fs.mkdir(renxDir, { recursive: true })

      const commandDir = path.join(renxDir, "command")
...
test("loads commands from .renx/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const renxDir = path.join(dir, ".renx")
      await fs.mkdir(renxDir, { recursive: true })

      const commandsDir = path.join(renxDir, "commands")
...
test("does not try to install dependencies in read-only RENX_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.RENX_CONFIG_DIR
  process.env.RENX_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.RENX_CONFIG_DIR
    else process.env.RENX_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable RENX_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "renx.json"),
        JSON.stringify({ $schema: "https://renx.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .renx config
      const projectDir = path.join(dir, "project")
      const renxDir = path.join(projectDir, ".renx")
      await fs.mkdir(renxDir, { recursive: true })
      ...
      // Local .renx config with different plugins
      await Filesystem.write(
        path.join(renxDir, "renx.json"),
        JSON.stringify({
          $schema: "https://renx.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .renx config with some overlapping plugins
      await Filesystem.write(
        path.join(renxDir, "renx.json"),
        JSON.stringify({
          $schema: "https://renx.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".renx")
...
// Note: preload.ts sets RENX_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .renx config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .renx directory config enables it
      const renxDir = path.join(dir, ".renx")
      await fs.mkdir(renxDir, { recursive: true })
      await Filesystem.write(
        path.join(renxDir, "renx.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/renx")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/renx")
    ...
    if (urlStr.includes(".well-known/renx")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/renx.json",
              headers: {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/renx")
            expect(remoteFetchedUrl).toBe("https://config.example.com/renx.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "renx.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-renx@2.4.3", file)).toBe("oh-my-renx@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-renx@2.4.3", "file:///project/.renx/plugin/oh-my-renx.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.renx/plugin/demo.ts", "file:///project/.renx/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.renx/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const renxDir = path.join(projectDir, ".renx")
        const pluginDir = path.join(renxDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("RENX_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["RENX_DISABLE_PROJECT_CONFIG"]
    process.env["RENX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["RENX_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["RENX_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .renx/ directories when flag is set", async () => {
    const originalEnv = process.env["RENX_DISABLE_PROJECT_CONFIG"]
    process.env["RENX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .renx directory with a command
          const renxDir = path.join(dir, ".renx", "command")
          await fs.mkdir(renxDir, { recursive: true })
          await Filesystem.write(path.join(renxDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .renx should NOT be in directories list
          const hasProjectRenx = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectRenx).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["RENX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["RENX_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["RENX_CONFIG_DIR"]
      process.env["RENX_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["RENX_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["RENX_CONFIG_DIR"]
      } else {
        process.env["RENX_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("RENX_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["RENX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["RENX_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["RENX_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["RENX_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from RENX_CONFIG_DIR, not project
...
describe("RENX_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in RENX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["RENX_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["RENX_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://renx.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["RENX_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["RENX_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in RENX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["RENX_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Renx Managed",
          PayloadIdentifier: "ai.renx.managed.test",
          PayloadType: "ai.renx.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://renx.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://renx.ai/config.json")
})
