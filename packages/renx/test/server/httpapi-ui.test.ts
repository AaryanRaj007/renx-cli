...
const original = {
  ...
  RENX_DISABLE_EMBEDDED_WEB_UI: Flag.RENX_DISABLE_EMBEDDED_WEB_UI,
  RENX_SERVER_PASSWORD: Flag.RENX_SERVER_PASSWORD,
  RENX_SERVER_USERNAME: Flag.RENX_SERVER_USERNAME,
  envPassword: process.env.RENX_SERVER_PASSWORD,
  envUsername: process.env.RENX_SERVER_USERNAME,
}

afterEach(() => {
  Flag.RENX_EXPERIMENTAL_HTTPAPI = original.RENX_EXPERIMENTAL_HTTPAPI
  Flag.RENX_DISABLE_EMBEDDED_WEB_UI = original.RENX_DISABLE_EMBEDDED_WEB_UI
  Flag.RENX_SERVER_PASSWORD = original.RENX_SERVER_PASSWORD
  Flag.RENX_SERVER_USERNAME = original.RENX_SERVER_USERNAME
  restoreEnv("RENX_SERVER_PASSWORD", original.envPassword)
  restoreEnv("RENX_SERVER_USERNAME", original.envUsername)
})
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            RENX_SERVER_PASSWORD: input?.password,
            RENX_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  test("serves the web UI through the experimental backend", async () => {
    Flag.RENX_EXPERIMENTAL_HTTPAPI = true
    Flag.RENX_DISABLE_EMBEDDED_WEB_UI = true
    let proxiedUrl: string | undefined

    const response = await uiApp({
      client: httpClient(
        new Response("<html>renx</html>", { headers: { "content-type": "text/html" } }),
        (request) => {
    ...
    expect(await response.text()).toBe("<html>renx</html>")
    expect(proxiedUrl).toBe("https://app.renx.ai/")
  })
  ...
  test("strips upstream transfer encoding headers from proxied assets", async () => {
    ...
    expect(response.status).toBe(200)
    expect(proxiedUrl).toBe("https://app.renx.ai/assets/app.js")
  ...
  test("requires server password for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "renx" }).request("/")
  ...
  test("accepts auth token for the web UI", async () => {
    ...
    const response = await uiApp({
      password: "secret",
      username: "renx",
      client: httpClient(new Response("<html>renx</html>", { headers: { "content-type": "text/html" } })),
    }).request(`/?auth_token=${btoa("renx:secret")}`)
  ...
  test("accepts basic auth for the web UI", async () => {
    ...
    const response = await uiApp({ password: "secret", username: "renx" }).request("/", {
      headers: { authorization: `Basic ${btoa("renx:secret")}` },
    })
  ...
  test("allows web UI preflight without auth", async () => {
    ...
    const response = await app({ password: "secret", username: "renx" }).request("/", {
      method: "OPTIONS",
      headers: {
