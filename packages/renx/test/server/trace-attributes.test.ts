...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under renx.", () => {
    expect(paramToAttributeKey("name")).toBe("renx.name")
    expect(paramToAttributeKey("slug")).toBe("renx.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["renx.sessionID"]).toBeUndefined()
    expect(attrs["renx.messageID"]).toBeUndefined()
    expect(attrs["renx.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under renx. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["renx.name"]).toBe("exa")
