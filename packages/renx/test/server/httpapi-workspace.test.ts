...
const originalHttpApi = Flag.RENX_EXPERIMENTAL_HTTPAPI
...
function request(path: string, directory: string, init: RequestInit = {}, httpApi = true) {
  return Effect.promise(() => {
    Flag.RENX_EXPERIMENTAL_HTTPAPI = httpApi
    const headers = new Headers(init.headers)
    headers.set("x-renx-directory", directory)
...
afterEach(async () => {
  ...
  Flag.RENX_EXPERIMENTAL_HTTPAPI = originalHttpApi
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-renx-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://renx.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-renx-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-renx-workspace")
      } finally {
