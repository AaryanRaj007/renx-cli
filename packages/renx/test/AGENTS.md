...
- `config?: Partial<Config.Info>` - Write an `renx.json` config file
...
- Directories are created in the system temp folder with prefix `renx-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("renx-test-")
  }),
)
```
