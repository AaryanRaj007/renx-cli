...
import { RenxClient } from "@renx/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<RenxClient["event"]["subscribe"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<RenxClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<RenxClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<RenxClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://renx.test"),
...
function sdk(
  input: {
    stream?: EventStream
    subscribe?: RenxClient["event"]["subscribe"]
    promptAsync?: RenxClient["session"]["promptAsync"]
    status?: RenxClient["session"]["status"]
    messages?: RenxClient["session"]["messages"]
    children?: RenxClient["session"]["children"]
    permissions?: RenxClient["permission"]["list"]
    questions?: RenxClient["question"]["list"]
  } = {},
) {
  const client = new RenxClient()

  const subscribe: RenxClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const promptAsync: RenxClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: RenxClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: RenxClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: RenxClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: RenxClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: RenxClient["question"]["list"] = input.questions ?? (() => ok([]))
