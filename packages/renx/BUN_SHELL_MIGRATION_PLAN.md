...
Replace runtime Bun shell template-tag usage in `packages/renx/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/renx/src` is removed except:
