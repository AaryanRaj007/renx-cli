 ...
 *
 * The script intentionally isolates `RENX_DB` before importing modules that touch
...
const preserveExerciseGlobalRoot = !!process.env.RENX_HTTPAPI_EXERCISE_GLOBAL
const exerciseGlobalRoot =
  process.env.RENX_HTTPAPI_EXERCISE_GLOBAL ??
  path.join(process.env.TMPDIR ?? "/tmp", `renx-httpapi-global-${process.pid}`)
process.env.XDG_DATA_HOME = path.join(exerciseGlobalRoot, "data")
process.env.XDG_CONFIG_HOME = path.join(exerciseGlobalRoot, "config")
process.env.XDG_STATE_HOME = path.join(exerciseGlobalRoot, "state")
process.env.XDG_CACHE_HOME = path.join(exerciseGlobalRoot, "cache")
process.env.RENX_DISABLE_SHARE = "true"
const exerciseConfigDirectory = path.join(exerciseGlobalRoot, "config", "renx")
const exerciseDataDirectory = path.join(exerciseGlobalRoot, "data", "renx")

const preserveExerciseDatabase = !!process.env.RENX_HTTPAPI_EXERCISE_DB
const exerciseDatabasePath =
  process.env.RENX_HTTPAPI_EXERCISE_DB ??
  path.join(process.env.TMPDIR ?? "/tmp", `renx-httpapi-exercise-${process.pid}.db`)
process.env.RENX_DB = exerciseDatabasePath
Flag.RENX_DB = exerciseDatabasePath

void (await import("@renx/core/util/log")).init({ print: false })
...
const original = {
  RENX_EXPERIMENTAL_HTTPAPI: Flag.RENX_EXPERIMENTAL_HTTPAPI,
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "renx.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "renx.jsonc")).text(),
          )
  ...
  http.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-renx-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-renx-directory")
  }),
  ...
  http
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "renx", model: "test" })}`,
      headers: ctx.headers(),
    }))
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-renx-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
...
function app(modules: Runtime, backend: Backend) {
  Flag.RENX_EXPERIMENTAL_HTTPAPI = backend === "effect"
    ...
    modules.ExperimentalHttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ RENX_SERVER_PASSWORD: undefined, RENX_SERVER_USERNAME: undefined }),
        ),
      ),
    ),
    { disableLogger: true },
...
const resetState = Effect.promise(async () => {
  ...
  Flag.RENX_EXPERIMENTAL_HTTPAPI = original.RENX_EXPERIMENTAL_HTTPAPI
