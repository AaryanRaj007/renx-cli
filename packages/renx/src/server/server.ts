...
function createHono(opts: CorsOptions, selection: ServerBackend.Selection = ServerBackend.force(select(), "hono")) {
  ...
  if (Flag.RENX_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.RENX_WORKSPACE_ID ? WorkspaceID.make(Flag.RENX_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
 ...
 * `PublicApi` is `RenxHttpApi` annotated with the `matchLegacyOpenApi`
...
async function listenHttpApi(opts: ListenOptions, selection: ServerBackend.Selection): Promise<Listener> {
  log.info("server backend selected", {
    ...
    "renx.server.runtime": HttpApiServer.name,
  })
