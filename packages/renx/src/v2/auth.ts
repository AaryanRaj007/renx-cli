...
import { Identifier } from "@renx/core/util/identifier"
import { NonNegativeInt, withStatics } from "@/util/schema"
import { Global } from "@renx/core/global"
import { AppFileSystem } from "@renx/core/filesystem"

export const OAUTH_DUMMY_KEY = "renx-oauth-dummy-key"
...
export class Service extends Context.Service<Service, Interface>()("@renx/v2/Auth") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Writable, AuthError> = Effect.fnUntraced(function* () {
      if (process.env.RENX_AUTH_CONTENT) {
        try {
          return JSON.parse(process.env.RENX_AUTH_CONTENT)
        } catch {}
      }
