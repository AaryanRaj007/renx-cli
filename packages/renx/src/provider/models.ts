...
import { Flock } from "@renx/core/util/flock"
...
export class Service extends Context.Service<Service, Interface>()("@renx/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.RENX_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.RENX_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.RENX_DISABLE_MODELS_FETCH) return {}
      // Flock is cross-process: concurrent renx CLIs can race on this cache file.
    ...
    if (!Flag.RENX_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
