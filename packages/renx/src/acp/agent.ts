...
import type { AssistantMessage, Event, RenxClient, SessionMessageResponse, ToolPart } from "@renx/sdk/v2"
...
export function init({ sdk: _sdk }: { sdk: RenxClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `renx auth login` in the terminal",
      name: "Login with renx",
      id: "renx-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Renx stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const renxProvider = providers.find((p) => p.id === "renx")
  if (renxProvider) {
    const [best] = Provider.sort(Object.values(renxProvider.models))
    if (best) {
      return {
