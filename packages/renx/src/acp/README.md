...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for renx.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal renx sessions
...
```bash
RENX_ENABLE_QUESTION_TOOL=1 renx acp
```
...
- **Terminal Integration**: Full terminal support via renx's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | renx acp
```
...
ACP sessions map cleanly to renx's internal session model:
