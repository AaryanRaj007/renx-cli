import * as Log from "@renx/core/util/log"
...
import { NamedError } from "@renx/core/util/error"
import { Flag } from "@renx/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { type InstanceContext } from "../project/instance"
import { InstallationLocal, InstallationVersion } from "@renx/core/installation/version"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in renx config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for renx serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://renx.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://renx.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@renx/Config") {}

function globalConfigFile() {
  const candidates = ["renx.jsonc", "renx.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://renx.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "renx.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "renx.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://renx.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "RENX_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/renx` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/renx`))
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://renx.ai/config.json"
            const source = `${url}/.well-known/renx`
        ...
        if (Flag.RENX_CONFIG) {
          yield* merge(Flag.RENX_CONFIG, yield* loadFile(Flag.RENX_CONFIG))
          log.debug("loaded custom config", { path: Flag.RENX_CONFIG })
        }

        if (!Flag.RENX_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("renx", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.RENX_CONFIG_DIR) {
          log.debug("loading config from RENX_CONFIG_DIR", { path: Flag.RENX_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".renx") || dir === Flag.RENX_CONFIG_DIR) {
            for (const file of ["renx.json", "renx.jsonc"]) {
          ...
          // Auto-discovered plugins under `.renx/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.RENX_CONFIG_CONTENT) {
          const source = "RENX_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.RENX_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from RENX_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["RENX_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("RENX_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.RENX_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.RENX_PERMISSION))
        }
        ...
        if (Flag.RENX_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.RENX_DISABLE_PRUNE) {
