...
const MANAGED_PLIST_DOMAIN = "ai.renx.managed"

// Keys injected by macOS/MDM into the managed plist that are not Renx config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/renx"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "renx")
    default:
      return "/etc/renx"
  }
}
...
export function managedConfigDir() {
  return process.env.RENX_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
