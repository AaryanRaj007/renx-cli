...
import { createRenxClient } from "@renx/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "renx.mode": input.resolveSession ? "local" : "attach",
      "renx.initial_input": !!input.initialInput,
      "renx.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "renx.directory": ctx.directory,
        "renx.resume": ctx.resume === true,
        "renx.agent.name": state.agent,
        "renx.model.provider": state.model?.providerID,
        "renx.model.id": state.model?.modelID,
        "renx.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "renx.model.provider": model.providerID,
            "renx.model.id": model.modelID,
            "renx.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "renx.model.id": state.model?.modelID,
                "renx.model.variant": state.activeVariant,
                "renx.prompt.chars": prompt.text.length,
                "renx.prompt.parts": prompt.parts.length,
                "renx.prompt.include_files": includeFiles,
                "renx.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "renx.directory": input.directory,
      "renx.initial_input": !!input.initialInput,
      "renx.demo": input.demo,
    },
    async () => {
