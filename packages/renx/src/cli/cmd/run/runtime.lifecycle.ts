...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "renx.agent.name": input.agent,
      "renx.directory": input.directory,
      "renx.first": input.first,
      "renx.model.provider": input.model?.providerID,
      "renx.model.id": input.model?.modelID,
      "renx.model.variant": input.variant,
