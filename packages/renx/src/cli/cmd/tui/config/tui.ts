...
export class Service extends Context.Service<Service, Interface>()("@renx/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old renx.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.renx`
  // folders between cwd and home, and RENX_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.RENX_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit RENX_TUI_CONFIG override, if set.
  if (Flag.RENX_TUI_CONFIG) {
    const configFile = Flag.RENX_TUI_CONFIG
  ...
  // 4. `.renx` directories (and RENX_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".renx") || dir === Flag.RENX_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".renx") && dir !== Flag.RENX_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    // `keybinds` is deprecated and will be removed in renx v2.0. Keep it
