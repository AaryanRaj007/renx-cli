...
const TUI_SCHEMA_URL = "https://renx.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from renx.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const renx = await renxFiles(input)
  for (const file of renx) {
...
async function renxFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "renx"),
    ...(await Filesystem.findUp(["renx.json", "renx.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "renx"))
  }
  if (Flag.RENX_CONFIG) files.push(Flag.RENX_CONFIG)
