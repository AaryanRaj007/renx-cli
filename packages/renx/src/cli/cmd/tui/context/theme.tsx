...
import renx from "./theme/renx.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@renx/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "renx")
        draft.active = typeof active === "string" ? active : "renx"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.renx, store.mode)
    })
