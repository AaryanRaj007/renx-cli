...
import type { AssistantMessage, FilePart, UserMessage } from "@renx/sdk/v2"
...
import { useBindings, useCommandShortcut, useLeaderActive, useRenxKeymap } from "../../keymap"
...
export function Prompt(props: PromptProps) {
  ...
  const usage = createMemo(() => {
    ...
    const limitText = model?.limit.context ? ` / ${Locale.number(model.limit.context)}` : ""
    return {
      context: pct ? `${Locale.number(tokens)}${limitText} (${pct})` : `${Locale.number(tokens)}${limitText}`,
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.RENX_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
    ...
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false}>
        <box flexDirection="column" width="100%">
          <box flexDirection="row" paddingRight={2} flexShrink={0} flexGrow={1}>
            <text fg={theme.textMuted} paddingRight={1}>›</text>
            <box flexGrow={1}>
            <textarea
              ...
              <box gap={2} flexDirection="row">
                ...
                <Switch>
                  ...
                  <Match when={store.mode === "shell"}>
                    <text fg={theme.textMuted}>esc exit shell mode</text>
                  </Match>
                </Switch>
              </box>
            </Show>
