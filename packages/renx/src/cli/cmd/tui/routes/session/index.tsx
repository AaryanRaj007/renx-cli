  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}renx -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  return (
    <>
      <Show when={text()}>
        ...
        >
          <text fg={theme.accent} attributes={TextAttributes.BOLD}>you</text>
          <box
            onMouseOver={() => {
              setHover(true)
            }}
  ...
  return (
    <>
      <box paddingLeft={1} marginTop={1}>
        <text fg={theme.accent} attributes={TextAttributes.BOLD}>renx</text>
      </box>
...
function parseMarkdownSegments(text: string): MarkdownSegment[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (markdownText) {
      segments.push({ type: "markdown", text: markdownText })
    }
  ...
  if (remainingText) {
    segments.push({ type: "markdown", text: remainingText })
  }
...
function normalizeLanguage(lang?: string): string {
  ...
  if (["js", "javascript", "jsx", "javascriptreact"].includes(l)) return "typescript"
  if (["ts", "typescript", "tsx", "typescriptreact"].includes(l)) return "typescript"
  if (["py", "python"].includes(l)) return "python"
  if (["sh", "bash", "zsh", "shell", "shellscript"].includes(l)) return "bash"
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  const segments = createMemo(() => parseMarkdownSegments(props.part.text))

  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={1} marginTop={1} flexShrink={0} flexDirection="column">
        <For each={segments()}>
          {(segment) => {
            if (segment.type === "code") {
              return (
                ...
                >
                  <code
                    filetype={normalizeLanguage(segment.language)}
                    drawUnstyledText={false}
                    streaming={true}
                    syntaxStyle={syntax()}
            ...
            return (
              <Switch>
                <Match when={Flag.RENX_EXPERIMENTAL_MARKDOWN}>
                  <markdown
                    syntaxStyle={syntax()}
                    streaming={true}
