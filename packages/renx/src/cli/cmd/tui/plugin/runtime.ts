...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".renx"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".renx", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".renx", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved }) {
  ...
  try {
    await WithInstance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.RENX_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.RENX_PURE && config.plugin_origins?.length) {
