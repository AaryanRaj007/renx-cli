...
//      renx server and runs interactive mode against it.
//
...
import { createRenxClient, type RenxClient, type ToolPart } from "@renx/sdk/v2"
import { Agent } from "@/agent/agent"
import { Permission } from "@/permission"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

const runtimeTask = import("./run/runtime")
type ModelInput = Parameters<RenxClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run renx with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running renx server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to RENX_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to RENX_SERVER_USERNAME or 'renx')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: RenxClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: RenxClient, sessionID: string) {
        const cfg = await sdk.config.get()
        if (!cfg.data) return
        if (cfg.data.share !== "auto" && !Flag.RENX_AUTO_SHARE && !args.share) return
      ...
      async function current(sdk: RenxClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: RenxClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: RenxClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: RenxClient) {
        ...
        async function loop(client: RenxClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createRenxClient({
        baseUrl: "http://renx.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
