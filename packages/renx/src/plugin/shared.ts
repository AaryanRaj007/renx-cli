...
export const DEPRECATED_PLUGIN_PACKAGES = ["renx-openai-codex-auth", "renx-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, renxVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(renxVersion) || semver.major(renxVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.renx
  if (typeof range !== "string") return
  if (!semver.satisfies(renxVersion, range)) {
    throw new Error(`Plugin requires renx ${range} but running ${renxVersion}`)
  }
}
