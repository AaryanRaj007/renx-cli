...
import { createRenx, type ToolPart } from "@renx/sdk"
...
console.log("🚀 Starting renx server...")
const renx = await createRenx({
  port: 0,
})
console.log("✅ Renx server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await renx.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new renx session...")
    const { client, server } = renx
    ...
    console.log("✅ Created renx session:", createResult.data.id)
  ...
  console.log("📝 Sending to renx:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Renx response:", JSON.stringify(result, null, 2))
