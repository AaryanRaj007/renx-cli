---
title: المزوّدون
description: استخدام أي موفّر LLM في Renx.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Renx ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Renx.
...
في `~/.local/share/renx/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Renx.
...
Renx Zen هي قائمة نماذج يوفّرها فريق Renx وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Renx. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Renx Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر renx، ثم انتقل إلى [renx.ai/auth](https://renx.ai/auth).
...
يعمل مثل أي مزوّد آخر في Renx واستخدامه اختياري بالكامل.

---

## Renx Go

Renx Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Renx، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Renx.

1. شغّل الأمر `/connect` في TUI، واختر `Renx Go`، ثم انتقل إلى [renx.ai/auth](https://renx.ai/zen).
...
لاستخدام Amazon Bedrock مع Renx:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل renx:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `renx.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Renx غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين renx لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل renx بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات renx.
...
تخزين الرمز في مساحة تخزين مصادقة renx.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Renx نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Renx
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`renx.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع renx:
...
لاستخدام Google Vertex AI مع Renx:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر renx، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [renx-helicone-session](https://github.com/H2Shami/renx-helicone-session) لتسجيل كل محادثة Renx تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Renx مدرجة كجلسة مستقلة.
...
يمكنك ضبط renx لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط renx لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط renx لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Renx. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/renx) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Renx.
...
6. **مهم**: قبل استخدام نماذج السحابة في Renx، يجب سحب معلومات النموذج محليا:
...
Renx Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Renx. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Renx Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Renx Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Renx.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات renx.
...
5. يمكنك أيضا تخصيصها عبر إعدادات renx. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Renx:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات renx. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع renx. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `renx.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Renx بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `renx auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات renx وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات renx.
