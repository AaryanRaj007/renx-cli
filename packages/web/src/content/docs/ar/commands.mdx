...
أنشئ الملف `.renx/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Renx أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Renx:
...
- على مستوى النظام: `~/.config/renx/commands/`
- لكل مشروع: `.renx/commands/`
...
يتضمن renx عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
