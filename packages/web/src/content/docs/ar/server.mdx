---
title: الخادم
description: تفاعل مع خادم renx عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `renx serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل renx استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `renx.local` |
...
عيّن `RENX_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `renx`، أو عيّن `RENX_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `renx serve` و `renx web`.
...
عند تشغيل `renx` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم renx للتفاعل مع renx برمجيا.
:::

تتيح هذه البنية لـ renx دعم عدة عملاء وتمكّنك من التفاعل مع renx برمجيا.

يمكنك تشغيل `renx serve` لبدء خادم مستقل. إذا كانت واجهة renx في terminal (TUI)
قيد التشغيل، فسيبدأ `renx serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Renx لـ [IDE](/docs/ide).
...
يعرض خادم renx واجهات API التالية.
