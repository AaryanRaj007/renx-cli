...
مع Renx يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Renx سمتنا `renx`.
...
- يريدون أن يطابق Renx مظهر terminal لديهم
...
يدعم Renx نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/renx/themes/*.json` أو `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.renx/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.renx/themes/*.json`
