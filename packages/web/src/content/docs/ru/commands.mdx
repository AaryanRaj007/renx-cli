...
Создайте `.renx/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию renx или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем renx [config](/docs/config):
...
- Глобальный: `~/.config/renx/commands/`
- Для каждого проекта: `.renx/commands/`
...
renx включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
