...
Le skill degli agenti permettono a Renx di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Renx cerca in queste posizioni:

- Config di progetto: `.renx/skills/<name>/SKILL.md`
- Config globale: `~/.config/renx/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Renx risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.renx/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/renx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.renx/skills/git-release/SKILL.md` cosi':
...
Renx elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `renx.json`:
...
**Per agenti integrati** (in `renx.json`):
