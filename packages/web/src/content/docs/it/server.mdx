---
title: Server
description: Interagisci con il server renx via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `renx serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client renx.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `renx.local` |
...
Imposta `RENX_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `renx`, oppure imposta `RENX_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `renx serve` sia per `renx web`.
...
Quando esegui `renx` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server renx per interagire con renx in modo programmatico.
:::

Questa architettura permette a renx di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `renx serve` per avviare un server standalone. Se la TUI di renx e gia in esecuzione, `renx serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Renx.
...
Il server renx espone le seguenti API.
