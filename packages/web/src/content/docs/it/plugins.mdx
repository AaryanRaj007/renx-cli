---
title: Plugin
description: Scrivi plugin per estendere Renx.
---

I plugin ti permettono di estendere Renx agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Renx.
...
- `.renx/plugins/` - plugin a livello progetto
- `~/.config/renx/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/renx/node_modules/`.
...
1. Config globale (`~/.config/renx/renx.json`)
2. Config di progetto (`renx.json`)
3. Directory plugin globale (`~/.config/renx/plugins/`)
4. Directory plugin di progetto (`.renx/plugins/`)
...
Renx esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di renx per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere renx.
...
:::note
Se usi l'app desktop di Renx, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a renx di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a renx:
...
L'helper `tool` crea uno strumento personalizzato che renx può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in renx insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://renx.ai/docs/sdk) per i dettagli.
