...
Renx Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
Renx Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in Renx. Ti abboni a Renx Go e
...
Renx Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Renx Go funziona come qualsiasi altro provider in Renx.

1. Accedi a **<a href={console}>Renx Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Renx Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Renx Go.
:::
...
Renx Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Renx config
utilizza il formato `renx-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`renx-go/kimi-k2.6` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Renx.
