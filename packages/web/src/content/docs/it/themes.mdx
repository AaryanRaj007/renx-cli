...
Con Renx puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Renx usa il tema `renx`.
...
Renx include diversi temi integrati.
...
- Vuole che Renx corrisponda all'aspetto del terminale
...
Renx supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/renx/themes/*.json` o `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.renx/themes/*.json`
4. **Directory di lavoro corrente** - in `./.renx/themes/*.json`
