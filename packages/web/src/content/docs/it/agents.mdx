...
In Renx esistono due tipi di agenti: agenti primari e subagenti.
...
Renx include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Renx include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Renx include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `renx.json`:
...
- Globale: `~/.config/renx/agents/`
- Per progetto: `.renx/agents/`
...
Se non specifichi una temperature, Renx usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Renx sia per la config specifica del progetto.
...
L'ID modello nella configurazione Renx usa il formato `provider/model-id`. Per esempio, se stai usando [Renx Zen](/docs/zen), useresti `renx/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `renx models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/renx).
:::
