...
Puoi aggiungere strumenti esterni a Renx usando il _Model Context Protocol_, o MCP. Renx supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Renx Config](https://renx.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/renx`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Renx gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Renx:
...
Se il server richiede autenticazione, Renx ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `renx mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Renx memorizzera' i token in modo sicuro in `~/.local/share/renx/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Renx insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Renx come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Renx al tuo account Sentry.
