---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w renx.
---

import config from "../../../../config.mjs"
export const console = config.console

renx używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji renx.
...
w `~/.local/share/renx/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim renx
config.
...
Renx Zen to lista modeli dostarczonych przez zespół renx, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z renx. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Renx Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Renx Zen` i przejdź do [renx.ai/auth](https://renx.ai/zen).
...
Działa jak każdy inny dostawca w renx i jest całkowicie opcjonalny w użyciu.

---

## Renx Go

Renx Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół renx, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z renx.

1. Uruchom polecenie `/connect` w TUI, wybierz `Renx Go` i przejdź do [renx.ai/auth](https://renx.ai/zen).
...
Aby używać Amazon Bedrock z renx:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania renx:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `renx.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w renx nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować renx do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod renx działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji renx.
...
renx używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować renx
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`renx.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem renx:
...
Aby używać Google Vertex AI z renx:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania renx.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą renx, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [renx-helicone-session](https://github.com/H2Shami/renx-helicone-session), aby automatycznie rejestrować każdą konwersację renx jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja renx będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować renx tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować renx tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować renx tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem renx. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/renx).
:::
...
Aby korzystać z Ollama Cloud z renx:
...
3. Skopiuj klucz API do użycia w renx.
...
6. **Ważne**: Przed użyciem modeli chmurowych w renx musisz pobrać informacje o modelu lokalnie:
...
Renx Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół renx. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Renx Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Renx Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API renx.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji renx.
...
5. Możesz je także dostosować za pomocą konfiguracji renx. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z renx:
...
Możesz także dostosować modele za pomocą konfiguracji renx. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z renx. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `renx.json` w katalogu projektu:
...
Pola `limit` pozwalają renx zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `renx auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację renx i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji renx.
