...
Możesz dodać dodatkowe informacje do renx za pomocą _Model Context Protocol_, w skrócie MCP. renx obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [renx Config](https://renx.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/renx`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
renx automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, renx:
...
Jeśli serwer wymaga uwierzytelnienia, renx poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `renx mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji renx bezpieczne przechowa tokeny w `~/.local/share/renx/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w renx, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie renx, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć renx z kontem Sentry.
