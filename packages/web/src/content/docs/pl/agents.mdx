...
W Renx obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Renx zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Renx ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Renx ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `renx.json`:
...
- Globalnie: `~/.config/renx/agents/`
- W projekcie: `.renx/agents/`
...
Jeśli nie zostanie skonfigurowana, Renx używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Renx, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Renx w `provider/model-id`. Na przykład, [Renx Zen](/docs/zen), użyj `renx/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `renx models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/renx).
:::
