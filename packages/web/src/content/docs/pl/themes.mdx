...
Dzięki renx możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie renx używa naszego własnego motywu `renx`.
...
renx ma kilka wbudowanych motywów.
...
- Chcą, aby Renx pasował do wyglądu ich terminala
...
renx obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/renx/themes/*.json` lub `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.renx/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.renx/themes/*.json`
