...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Renx posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Renx.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `RENX_EXPERIMENTAL_LSP_TOOL=true` (lub `RENX_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Renx lub gdy zmienna środowiskowa `RENX_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Renx:
