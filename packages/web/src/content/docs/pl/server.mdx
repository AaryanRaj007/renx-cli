---
title: Serwer
description: Interakcja z serwerem renx za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `renx serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient renx.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `renx.local` |
...
Ustaw `RENX_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `renx` lub ustaw `RENX_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `renx serve`, jak i `renx web`.
...
Kiedy uruchomisz `renx`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera renx do programowej interakcji z renx.
:::

Ta architektura umożliwia obsługę wielu klientów przez renx i programową interakcję z kodem otwartym.

Możesz uruchomić `renx serve`, aby uruchomić samodzielny serwer. Jeśli masz
renx TUI działa, `renx serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki renx [IDE](/docs/ide).
...
Serwer renx udostępnia następujące interfejsy API.
