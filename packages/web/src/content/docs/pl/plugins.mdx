---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć renx.
---

Wtyczki umożliwiające wyłączenie renx poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania renx.
...
- `.renx/plugins/` - Wtyczki na poziomie projektu
- `~/.config/renx/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/renx/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/renx/renx.json`)
2. Konfiguracja projektu (`renx.json`)
3. Globalny katalog wtyczek (`~/.config/renx/plugins/`)
4. Katalog wtyczek projektu (`.renx/plugins/`)
...
renx uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z renx do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do renx.
...
:::note
Jeśli korzystasz z aplikacji komputerowej renx, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj renx czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do renx:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod renx. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://renx.ai/docs/sdk).
