...
Umiejętności agenta pozwalają renx odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
renx przeszukuje te lokalizacje:

- Project config: `.renx/skills/<name>/SKILL.md`
- Global config: `~/.config/renx/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu renx przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.renx/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/renx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.renx/skills/git-release/SKILL.md` w ten sposób:
...
renx wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `renx.json`:
...
**Dla agentów wbudowanych** (w `renx.json`):
