...
Du kan legge til eksterne verktøy i Renx ved å bruke _Model Context Protocol_ eller MCP. Renx støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Renx Config](https://renx.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="renx.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/renx`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Renx håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Renx:
...
Hvis serveren krever autentisering, vil Renx be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `renx mcp auth <server-name>`.
...
```bash
renx mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Renx lagre tokens sikkert i `~/.local/share/renx/mcp-auth.json`.
...
renx mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Renx, sammen med innebygde verktøy. Så du kan administrere dem gjennom Renx-konfigurasjonen som alle andre verktøy.
...
```bash
renx mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Renx til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Renx-samtale oppført som en separat økt.
