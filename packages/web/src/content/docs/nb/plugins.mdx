...
description: Skriv dine egne programtillegg for å utvide Renx.
---

Plugins lar deg utvide Renx ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Renx.
...
- `.renx/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/renx/node_modules/`.
...
1. Global konfigurasjon (`~/.config/renx/renx.json`)
2. Prosjektkonfigurasjon (`renx.json`)
3. Global plugin-katalog (`~/.config/renx/plugins/`)
4. Prosjektpluginkatalog (`.renx/plugins/`)
...
Renx kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Renx SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Renx.
...
:::note
Hvis du bruker Renx-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Renx fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Renx:
...
`tool`-hjelperen lager et tilpasset verktøy som Renx kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Renx sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://renx.ai/docs/sdk) for detaljer.
