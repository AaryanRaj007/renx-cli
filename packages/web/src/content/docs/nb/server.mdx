---
title: Tjener
description: Samhandle med Renx-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `renx serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Renx-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `renx.local` |
...
Sett `RENX_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `renx` som standard, eller sett `RENX_SERVER_USERNAME` for å overstyre det. Dette gjelder både `renx serve` og `renx web`.
...
Når du kjører `renx` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Renx-serveren til å samhandle med Renx programmatisk.
:::

Denne arkitekturen lar Renx støtte flere klienter og lar deg samhandle med Renx programmatisk.

Du kan kjøre `renx serve` for å starte en frittstående server. Hvis du har
Renx TUI kjører, vil `renx serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Renx [IDE](/docs/ide) plugins.
...
renx-serveren viser følgende APIer.
