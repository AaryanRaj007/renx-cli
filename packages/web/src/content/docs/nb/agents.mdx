...
Det er to typer agenter i Renx; primære agenter og underagenter.
...
Renx kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Renx kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Renx kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `renx.json`:
...
- Globalt: `~/.config/renx/agents/`
- Per prosjekt: `.renx/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Renx modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Renx-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Renx-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Renx Zen](/docs/zen), vil du bruke `renx/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `renx models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/renx).
:::
