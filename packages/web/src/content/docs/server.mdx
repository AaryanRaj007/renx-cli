---
title: Server
description: Interact with renx server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `renx serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an renx client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `renx.local` |
...
Set `RENX_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `renx`, or set `RENX_SERVER_USERNAME` to override it. This applies to both `renx serve` and `renx web`.
...
When you run `renx` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the renx server to interact with renx programmatically.
:::

This architecture lets renx support multiple clients and allows you to interact with renx programmatically.

You can run `renx serve` to start a standalone server. If you have the
renx TUI running, `renx serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Renx [IDE](/docs/ide) plugins.
...
The renx server exposes the following APIs.
