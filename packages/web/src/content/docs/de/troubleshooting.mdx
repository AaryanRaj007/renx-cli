...
Wenn Renx Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\renx\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `renx --log-level DEBUG`.

---

## Speicher

renx speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/renx/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\renx`
...
Renx Desktop startet im Hintergrund einen lokalen Renx-Server (`renx-cli`-Sidecar).
...
- Nur macOS: `Renx`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/renx/renx.jsonc` (oder `~/.config/renx/renx.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/renx/renx.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\renx\renx.jsonc`
...
Renx kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/renx/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\renx\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Renx ihn neu aufbauen kann.

1. Beende Renx Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/renx`
- **Linux**: loesche `~/.cache/renx` (oder fuehre aus: `rm -rf ~/.cache/renx`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\renx`

3. Starte Renx Desktop neu.

---

### Server-Verbindungsprobleme beheben

Renx Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `renx.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `RENX_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `RENX_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Renx Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Renx-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Renx Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Renx in den OS-Einstellungen aktiviert sind, und
...
1. Beende Renx Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Renx Desktop):

- `renx.settings.dat` (desktop default server URL)
- `renx.global.dat` und `renx.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Renx hast:
...
3. Stelle sicher, dass du die neueste Version hast: `renx upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `renx models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\renx`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. renx installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\renx`

2. Starte renx neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt renx, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
renx erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
