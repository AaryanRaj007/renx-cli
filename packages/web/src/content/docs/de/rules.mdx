---
title: Regeln
description: Lege eigene Anweisungen fuer renx fest.
---

Du kannst renx ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in renx den Befehl `/init` aus.
...
So kann renx sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht renx sie zu erweitern.
...
renx kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/renx/AGENTS.md` speichern.
Diese gelten in allen renx-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Renx dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/renx/AGENTS.md` existiert)
...
Beim Start sucht renx in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/renx/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `renx.json` oder in `~/.config/renx/renx.json` angeben.
...
renx parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit renx.json

Empfohlen ist das `instructions`-Feld in der `renx.json`:
...
Du kannst renx explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `renx.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
