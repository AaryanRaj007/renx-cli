---
title: LSP Server
description: Renx lässt sich in Ihren LSP-Server integrieren.
---

Renx lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

Renx verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `RENX_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn Renx eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer Renx-Konfiguration anpassen.
