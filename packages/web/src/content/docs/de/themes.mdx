...
Mit Renx kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Renx das `renx`-Theme.
...
Renx liefert mehrere eingebaute Themes mit.
...
- Renx optisch ans Terminal anpassen wollen
...
Renx unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/renx/themes/*.json` oder `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.renx/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.renx/themes/*.json`
