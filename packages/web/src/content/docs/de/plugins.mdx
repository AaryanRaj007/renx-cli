---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Renx zu erweitern.
---

Mit Plugins können Sie Renx erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Renx zu ändern.
...
- `.renx/plugins/` – Plugins auf Projektebene
- `~/.config/renx/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/renx/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/renx/renx.json`)
2. Projektkonfiguration (`renx.json`)
3. Globales Plugin-Verzeichnis (`~/.config/renx/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.renx/plugins/`)
...
Renx führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Renx-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Renx erweitern können.
...
:::note
Wenn Sie die Renx-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Renx `.env`-Dateien liest:
...
Plugins können Renx auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Renx aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Renx zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://renx.ai/docs/sdk).
