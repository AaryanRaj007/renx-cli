---
title: Server
description: Nutze den renx-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `renx serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein renx-Client nutzen kann.
...
Setze `RENX_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `renx`, kann aber mit `RENX_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `renx serve` und `renx web`.
...
Wenn du `renx` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den renx-Server, um renx programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `renx serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `renx serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Renx-[IDE](/docs/ide)-Plugins.
...
Der renx-Server stellt folgende APIs bereit.
