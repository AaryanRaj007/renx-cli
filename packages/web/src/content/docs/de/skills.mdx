...
Agent Skills erlauben Renx, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Renx sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Renx vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.renx/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/renx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.renx/skills/git-release/SKILL.md` zum Beispiel so:
...
Renx listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `renx.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `renx.json`):
