---
title: TUI
description: Using the Renx terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Renx provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Renx starts the TUI for the current directory.
...
When using the Renx TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Renx. Allows you to select from available providers and add their API keys.
...
This is separate from `renx.json`, which configures server/runtime behavior.
...
Use `RENX_TUI_CONFIG` to load a custom TUI config path.
