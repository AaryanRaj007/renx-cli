---
title: Config
description: Using the Renx JSON config.
---

You can configure Renx using a JSON config file.

---

## Format

Renx supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="renx.jsonc"
{
...
1. **Remote config** (from `.well-known/renx`) - organizational defaults
2. **Global config** (`~/.config/renx/renx.json`) - user preferences
3. **Custom config** (`RENX_CONFIG` env var) - custom overrides
4. **Project config** (`renx.json` in project) - project-specific settings
5. **`.renx` directories** - agents, commands, plugins
6. **Inline config** (`RENX_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/renx/` on macOS) - admin-controlled
...
:::note
The `.renx` and `~/.config/renx` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/renx` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/renx"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Renx config in `~/.config/renx/renx.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/renx/tui.json`.
...
Add `renx.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Renx starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
Specify a custom config file path using the `RENX_CONFIG` environment variable.

```bash
export RENX_CONFIG=/path/to/my/custom-config.json
renx run "Hello world"
```
...
Specify a custom config directory using the `RENX_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.renx` directory, and should
follow the same structure.

```bash
export RENX_CONFIG_DIR=/path/to/my/config-directory
renx run "Hello world"
```

The custom directory is loaded after the global config and `.renx` directories, so it **can override** their settings.
...
Drop an `renx.json` or `renx.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/renx/` |
| Linux    | `/etc/renx/`                         |
| Windows  | `%ProgramData%\renx`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Renx reads managed preferences from the `ai.renx.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Renx checks these paths:

1. `/Library/Managed Preferences/<user>/ai.renx.managed.plist`
2. `/Library/Managed Preferences/ai.renx.managed.plist`

The plist keys map directly to `renx.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.renx.managed` PayloadType. The Renx config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.renx.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.renx.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.renx</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`renx.ai/config.json`**](https://renx.ai/config.json).

TUI config uses [**`renx.ai/tui.json`**](https://renx.ai/tui.json).
...
Use `RENX_TUI_CONFIG` to point to a custom TUI config file.

Legacy `theme`, `keybinds`, and `tui` keys in `renx.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `renx serve` and `renx web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Renx server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `renx.local`. Useful for running multiple instances on the same network.
...
If not specified, Renx will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Renx config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Renx tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
You can also define agents using markdown files in `~/.config/renx/agents/` or `.renx/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Renx will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`renx run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/renx/commands/` or `.renx/commands/`. [Learn more here](/docs/commands).
...
Renx uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Renx will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, renx **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Renx with custom tools, hooks, and integrations.

Place plugin files in `.renx/plugins/` or `~/.config/renx/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="renx.json"
{
  "$schema": "https://renx.ai/config.json",
  "plugin": ["renx-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Renx to only use specific providers rather than disabling them one by one.
