...
Renx Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Renx Go şu anda beta olarak mevcut.
:::

Go, Renx'daki diğer sağlayıcılar gibi çalışır. Renx Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Renx'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Renx Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Renx Go, Renx'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Renx Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Renx Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Renx Go'ya abone olabilir.
:::
...
Renx Go aşağıdaki limitleri içerir:
...
Renx yapılandırmanızdaki [model id](/docs/config/#models) formatı `renx-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `renx-go/kimi-k2.6` kullanmalısınız.
...
Renx Go'yu şu amaçlarla oluşturduk:
...
4. Renx ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
