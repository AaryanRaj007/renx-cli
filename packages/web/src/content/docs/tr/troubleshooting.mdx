...
renx ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\renx\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `renx --log-level DEBUG`.

---

## Depolama

renx, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/renx/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\renx` yapistirin
...
renx Desktop arka planda yerel bir renx sunucusu (`renx-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `renx` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/renx/renx.jsonc` (veya `~/.config/renx/renx.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/renx/renx.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\renx\renx.jsonc` yapistirin
...
renx diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/renx/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\renx\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), renx'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. renx Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/renx`
- **Linux**: `~/.cache/renx` dizinini silin (veya `rm -rf ~/.cache/renx` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\renx` yapistirin

3. renx Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

renx Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`renx.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `RENX_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `RENX_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta renx Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, renx ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

renx Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda renx icin bildirimler etkinse
...
1. renx Desktop'u kapatin
2. Su dosyalari bulun ve silin (renx Desktop uygulama veri dizininde yer alirlar):

- `renx.settings.dat` (desktop varsayilan sunucu URL'si)
- `renx.global.dat` ve `renx.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
renx ile ilgili bir sorun yasiyorsaniz:
...
3. `renx upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `renx models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\renx`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. renx, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\renx`

2. En guncel provider paketlerini yeniden kurmak icin renx'u yeniden baslatin

Bu, renx'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
renx, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
