...
description: Renx, LSP sunucularınızla bütünleşir.
---

Renx, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

Renx, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`RENX_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, renx yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
