...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. renx yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
renx içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `RENX_EXPERIMENTAL_LSP_TOOL=true` (veya `RENX_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca renx provider kullanılırken veya `RENX_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

renx başlatırken etkinleştirmek için:
