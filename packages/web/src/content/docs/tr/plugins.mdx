---
title: Eklentiler
description: Renx'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Renx'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Renx'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.renx/plugins/` - Proje düzeyinde eklentiler
- `~/.config/renx/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/renx/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/renx/renx.json`)
2. Project config (`renx.json`)
3. Global eklenti dizini (`~/.config/renx/plugins/`)
4. Proje eklenti dizini (`.renx/plugins/`)
...
Renx bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Renx SDK istemcisi.
...
Renx'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Renx masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Renx'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Renx'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Renx'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Renx'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://renx.ai/docs/sdk) bakın.
