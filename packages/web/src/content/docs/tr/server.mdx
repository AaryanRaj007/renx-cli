---
title: Sunucu
description: renx sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`renx serve` komutu, renx istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `renx.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `RENX_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `renx` değeridir; değiştirmek isterseniz `RENX_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `renx serve` hem de `renx web` için geçerlidir.
...
`renx` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
renx ile programatik etkileşim için renx sunucusunu kullanın.
:::

Bu mimari, renx'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`renx serve` ile bağımsız bir sunucu başlatabilirsiniz. renx TUI açıksa, `renx serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum renx [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
renx sunucusu aşağıdaki API'leri sunar.
