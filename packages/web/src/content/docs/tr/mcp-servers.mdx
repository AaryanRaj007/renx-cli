...
_Model Bağlam Protokolü_ veya MCP kullanarak renx'a harici araçlar ekleyebilirsiniz. renx hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [renx Config](https://renx.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/renx` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
renx, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde renx şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa renx, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `renx mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından renx, belirteçleri `~/.local/share/renx/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra renx'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi renx yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve renx'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
