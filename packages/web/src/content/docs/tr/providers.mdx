---
title: Sağlayıcılar
description: renx'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

renx, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı renx yapılandırmanızda yapılandırın.
...
`~/.local/share/renx/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları renx'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Renx Zen, renx ekibi tarafından sağlanan modellerin bir listesidir.
renx ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Renx Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, renx'u seçin ve [renx.ai/auth](https://renx.ai/auth)'ye gidin.
...
renx'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Renx Go

Renx Go, renx ile iyi çalıştığı test edilmiş ve doğrulanmış, renx ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Renx Go`'yu seçin ve [renx.ai/auth](https://renx.ai/zen) adresine gidin.
...
Amazon Bedrock'u renx ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   renx'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `renx.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi renx'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
renx'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   renx'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Renx yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in renx auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
renx, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. renx'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`renx.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`renx.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi renx'la kullanmak için:
...
Google Vertex AI'yi renx ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları renx çalıştırılırken ayarlayın.
...
Helicone'un renx aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her renx görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [renx-helicone-session](https://github.com/H2Shami/renx-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her renx görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
renx'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla renx'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla renx'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini renx için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/renx)'a bakın.
:::
...
Ollama Cloud'u renx ile kullanmak için:
...
3. renx'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: renx'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Renx Zen, renx ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Renx Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Renx Zen**'i arayın.

   ```txt
   /connect
   ```

3. renx API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   renx yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca renx yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Renx ile kullanmak için:
...
Modelleri renx yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı renx ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `renx.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları renx'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `renx auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için renx yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, renx yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
