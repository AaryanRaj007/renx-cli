---
title: TUI
description: Renx terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Renx, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Renx'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Renx TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Renx'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Renx'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `renx.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `RENX_TUI_CONFIG` kullanın.
