...
renx ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak renx kendi `renx` temasını kullanır.
...
renx birden fazla yerleşik temayla gelir.
...
- renx'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
renx, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/renx/themes/*.json` veya `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.renx/themes/*.json`
4. **Geçerli çalışma dizini** - `./.renx/themes/*.json`
