---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Renx.
---

Dodaci vam omogućavaju da proširite Renx spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Renx.
...
- `.renx/plugins/` - Dodaci na nivou projekta
- `~/.config/renx/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/renx/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/renx/renx.json`)
2. Konfiguracija projekta (`renx.json`)
3. Globalni direktorij dodataka (`~/.config/renx/plugins/`)
4. Direktorij dodataka projekta (`.renx/plugins/`)
...
Renx pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Renx SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Renx.
...
Spriječite renx da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u renx:
...
Pomoćnik `tool` kreira prilagođeni alat koji renx može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za renx zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://renx.ai/docs/sdk) za detalje.
