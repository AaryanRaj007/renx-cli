---
title: Konfiguracija
description: Korištenje Renx JSON konfiguracije.
---

Možete konfigurirati Renx koristeći JSON konfiguracijski fajl.

---

## Format

Renx podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/renx`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/renx/renx.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`RENX_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`renx.json` u projektu) - postavke specifične za projekat
5. **`.renx` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`RENX_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.renx` i `~/.config/renx` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/renx` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Renx konfiguraciju u `~/.config/renx/renx.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/renx/tui.json`.
...
Dodajte `renx.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Renx pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `RENX_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `RENX_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.renx` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.renx`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`renx.ai/config.json`**](https://renx.ai/config.json).

TUI konfiguracija koristi [**`renx.ai/tui.json`**](https://renx.ai/tui.json).
...
Koristite `RENX_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `renx.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `renx serve` i `renx web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Renx server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `renx.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Renx konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Renx pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/renx/agents/` ili `.renx/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Renx će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`renx run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/renx/commands/` ili `.renx/commands/`. [Saznajte više ovdje](/docs/commands).
...
Renx će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Renx **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Renx sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.renx/plugins/` ili `~/.config/renx/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Renx da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
