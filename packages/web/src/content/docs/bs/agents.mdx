...
Postoje dvije vrste agenata u Renx: primarni agenti i podagenti.
...
Renx dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Renx dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Renx dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `renx.json`:
...
- Globalno: `~/.config/renx/agents/`
- Po projektu: `.renx/agents/`
...
Ako temperatura nije navedena, Renx koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Renx konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Renx konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Renx Zen](/docs/zen), koristili biste `renx/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `renx models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/renx).
:::
