...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Renx dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Renx.
...
:::note
Ovaj alat je dostupan samo kada je `RENX_EXPERIMENTAL_LSP_TOOL=true` (ili `RENX_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Renx provajdera ili kada je varijabla `RENX_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Renx:
