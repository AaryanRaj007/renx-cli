---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Renx.
---

import config from "../../../../config.mjs"
export const console = config.console

Renx koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Renx konfiguraciji.
...
u `~/.local/share/renx/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Renx
config.
...
Renx Zen je lista modela koje je obezbedio Renx tim koji su bili
testirano i potvrđeno da dobro radi sa Renx. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Renx Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite renx i idite na [renx.ai/auth](https://renx.ai/auth).
...
Radi kao i svaki drugi provajder u Renx i potpuno je opcionalan za korištenje.

---

## Renx Go

Renx Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Renx tim i koji su testirani i verificirani da dobro rade s Renx-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Renx Go` i idite na [renx.ai/auth](https://renx.ai/zen).
...
Da biste koristili Amazon Bedrock s Renx:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete renx:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `renx.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Renx.
:::
...
Možete konfigurirati renx za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi renx ispravno radio.
   :::
...
Također možete dodati modele kroz svoju renx konfiguraciju.
...
da pohrani token u renx auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Renx koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Renx
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`renx.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s renx:
...
Za korištenje Google Vertex AI s Renx:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće renx.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem renxa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [renx-helicone-session](https://github.com/H2Shami/renx-helicone-session) da automatski prijavite svaki Renx razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Renx razgovor naveden kao zasebna sesija.
...
Možete konfigurirati renx za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati renx za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati renx za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Renx. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/renx) za detalje.
:::
...
Da biste koristili Ollama Cloud s Renx:
...
3. Kopirajte API ključ za korištenje u Renx.
...
6. **Važno**: Prije upotrebe modela oblaka u Renx, morate lokalno povući informacije o modelu:
...
Renx Zen je lista testiranih i verifikovanih modela koju je obezbedio Renx tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Renx Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Renx Zen**.

```txt
   /connect
```

3. Unesite svoj Renx API ključ.
...
Također možete dodati dodatne modele putem vaše renx konfiguracije.
...
5. Također ih možete prilagoditi putem vaše renx konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Renxom:
...
Također možete prilagoditi modele kroz svoju renx konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s renx-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `renx.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Renx da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `renx auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Renx konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj renx konfiguraciji.
