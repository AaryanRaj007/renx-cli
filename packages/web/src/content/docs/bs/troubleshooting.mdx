...
Da biste otklonili probleme s Renx, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\renx\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `renx --log-level DEBUG`.

---

## Pohrana

Renx pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/renx/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\renx`
...
Renx Desktop pokreće lokalni Renx server (`renx-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Renx` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/renx/renx.jsonc` (ili `~/.config/renx/renx.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/renx/renx.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\renx\renx.jsonc`
...
Renx također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/renx/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\renx\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Renx mogao ponovo izgraditi.

1. Potpuno zatvorite Renx Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/renx`
- **Linux**: obrišite `~/.cache/renx` (ili pokrenite `rm -rf ~/.cache/renx`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\renx`

3. Ponovo pokrenite Renx Desktop.

---

### Rješavanje problema sa vezom na serveru

Renx Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `renx.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `RENX_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `RENX_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Renx Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Renx karakteristikama.

---

### Obavještenja se ne prikazuju

Renx Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Renx u postavkama vašeg OS-a, i
...
1. Zatvorite Renx Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Renx Desktop aplikacije):

- `renx.settings.dat` (URL zadanog servera za desktop)
- `renx.global.dat` i `renx.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `renx upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `renx models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\renx`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Renx dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\renx`

2. Ponovo pokrenite Renx da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Renx da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Renx će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
