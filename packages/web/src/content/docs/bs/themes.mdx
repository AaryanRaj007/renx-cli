...
U Renx mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Renx koristi nasu `renx` temu.
...
Renx dolazi sa vise ugradenih tema.
...
- Zele da Renx odgovara izgledu njihovog terminala
...
Renx podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/renx/themes/*.json` ili `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.renx/themes/*.json`
4. **Trenutni radni direktorij** - `./.renx/themes/*.json`
