...
Možete dodati vanjske alate u Renx koristeći _Model Context Protocol_, ili MCP. Renx podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Renx Config](https://renx.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/renx`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Renx automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Renx će:
...
Ako server zahtijeva autentifikaciju, Renx će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `renx mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Renx sigurno čuva tokene u `~/.local/share/renx/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Renx, zajedno s ugrađenim alatima. Možete njima upravljati kroz Renx konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Renx sa vašim Sentry nalogom.
