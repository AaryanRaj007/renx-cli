---
title: Server
description: Komunicirajte s renx serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `renx serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji renx klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `renx.local` |
...
Postavite `RENX_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `renx`, ili postavite `RENX_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `renx serve` i za `renx web`.
...
Kada pokrenete `renx`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite renx server za programsku interakciju sa renx-om.
:::

Ova arhitektura omogucava renx podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `renx serve` da startate standalone server. Ako je renx TUI vec pokrenut, `renx serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Renx [IDE](/docs/ide) pluginovi.
...
renx server izlaže sljedece API-je.
