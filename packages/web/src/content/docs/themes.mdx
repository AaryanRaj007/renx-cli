...
With Renx you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Renx uses our own `renx` theme.
...
Renx comes with several built-in themes.
...
- Want Renx to match their terminal's appearance
...
Renx supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/renx/themes/*.json` or `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.renx/themes/*.json`
4. **Current working directory** - Defined in `./.renx/themes/*.json`
