...
Puede agregar herramientas externas a Renx utilizando el _Model Context Protocol_ o MCP. Renx admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Renx Config](https://renx.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/renx`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Renx maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Renx:
...
Si el servidor requiere autenticación, Renx le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `renx mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Renx almacenará los tokens de forma segura en `~/.local/share/renx/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Renx, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Renx como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Renx a su cuenta Sentry.
