---
title: Servidor
description: Interactuar con el servidor renx a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `renx serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente renx puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `renx.local` |
...
Configure `RENX_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `renx`, o configure `RENX_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `renx serve` como a `renx web`.
...
Cuando ejecuta `renx`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor renx para interactuar con renx mediante programación.
:::

Esta arquitectura permite que renx admita múltiples clientes y le permite interactuar con renx mediante programación.

Puede ejecutar `renx serve` para iniciar un servidor independiente. Si tienes el
renx TUI ejecutándose, `renx serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Renx [IDE](/docs/ide).
...
El servidor renx expone las siguientes API.
