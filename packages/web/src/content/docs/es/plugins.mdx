---
title: Complementos
description: Escriba sus propios complementos para extender Renx.
---

Los complementos le permiten extender Renx al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Renx.
...
- `.renx/plugins/` - Complementos a nivel de proyecto
- `~/.config/renx/plugins/` - Complementos globales
...
```json title="renx.json"
{
  ...
  "plugin": ["renx-helicone-session", "renx-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/renx/node_modules/`.
...
1. Configuración global (`~/.config/renx/renx.json`)
2. Configuración del proyecto (`renx.json`)
3. Directorio global de complementos (`~/.config/renx/plugins/`)
4. Directorio de complementos del proyecto (`.renx/plugins/`)
...
Renx ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".renx/plugins/my-plugin.ts"
...
```js title=".renx/plugins/example.js"
...
- `client`: Un cliente SDK renx para interactuar con la IA.
...
import type { Plugin } from "@renx-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar renx.
...
```js title=".renx/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "renx"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Renx, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que renx lea archivos `.env`:

```javascript title=".renx/plugins/env-protection.js"
...
```javascript title=".renx/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a renx:

```ts title=".renx/plugins/custom-tools.ts"
import { type Plugin, tool } from "@renx-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que renx puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para renx junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://renx.ai/docs/sdk) para obtener más detalles.
...
```ts title=".renx/plugins/compaction.ts"
import type { Plugin } from "@renx-ai/plugin"
...
```ts title=".renx/plugins/custom-compaction.ts"
import type { Plugin } from "@renx-ai/plugin"
