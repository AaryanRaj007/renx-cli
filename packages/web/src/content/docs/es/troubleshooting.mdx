...
Para depurar problemas con Renx, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/renx/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\renx\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `renx --log-level DEBUG`.

---

## Almacenamiento

renx almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/renx/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\renx`
...
Renx Desktop ejecuta un servidor Renx local (el sidecar `renx-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Renx` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/renx/renx.jsonc` (o `~/.config/renx/renx.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/renx/renx.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\renx\renx.jsonc`
...
Renx también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/renx/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\renx\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.renx/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Renx pueda reconstruirlo.

1. Salga de Renx Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/renx`
- **Linux**: eliminar `~/.cache/renx` (o ejecutar `rm -rf ~/.cache/renx`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\renx`

3. Reinicie el escritorio Renx.

---

### Solucionar problemas de conexión del servidor

Renx Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `renx.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `RENX_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `RENX_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Renx requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Renx.

---

### Notificaciones que no se muestran

Renx Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Renx en la configuración de su sistema operativo, y
...
1. Salga del escritorio Renx.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Renx):

- `renx.settings.dat` (URL del servidor predeterminado de escritorio)
- `renx.global.dat` y `renx.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/renx/issues**](https://github.com/anomalyco/renx/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**renx.ai/discord**](https://renx.ai/discord)
...
3. Asegúrese de tener la última versión con `renx upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `renx models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/renx
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\renx`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. renx instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\renx`

2. Reinicie renx para reinstalar los paquetes del proveedor más recientes.

Esto obligará a renx a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
renx detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
