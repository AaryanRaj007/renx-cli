...
description: Cree herramientas que LLM pueda llamar en renx.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de renx como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.renx/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/renx/tools/`.
...
```ts title=".renx/tools/database.ts" {1}
...
```ts title=".renx/tools/project.ts" {8}
import { tool } from "@renx-ai/plugin"
...
```python title=".renx/tools/add.py"
import sys
...
```ts title=".renx/tools/python-add.ts" {10}
import { tool } from "@renx-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".renx/tools/add.py")
