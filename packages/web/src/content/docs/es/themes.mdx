...
Con Renx puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Renx usa nuestro propio tema `renx`.
...
Renx viene con varios temas integrados.
...
- Quiere que Renx coincida con la apariencia de su terminal
...
Renx admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/renx/themes/*.json` o `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.renx/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.renx/themes/*.json`
