...
Las habilidades del agente permiten a Renx descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Renx busca estas ubicaciones:

- Configuración del proyecto: `.renx/skills/<name>/SKILL.md`
- Configuración global: `~/.config/renx/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Renx sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.renx/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/renx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.renx/skills/git-release/SKILL.md` así:
...
Renx enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `renx.json`:
...
**Para agentes integrados** (en `renx.json`):
