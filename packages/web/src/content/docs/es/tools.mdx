...
Las herramientas permiten que LLM realice acciones en su código base. Renx viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Renx.
...
:::note
Esta herramienta solo está disponible cuando `RENX_EXPERIMENTAL_LSP_TOOL=true` (o `RENX_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Renx o cuando la variable de entorno `RENX_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Renx:

```bash
RENX_ENABLE_EXA=1 renx
```
