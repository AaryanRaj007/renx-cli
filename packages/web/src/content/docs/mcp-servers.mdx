...
You can add external tools to Renx using the _Model Context Protocol_, or MCP. Renx supports both local and remote servers.
...
You can define MCP servers in your [Renx Config](https://renx.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/renx` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Renx automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Renx will:
...
If the server requires authentication, Renx will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `renx mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Renx will store the tokens securely in `~/.local/share/renx/mcp-auth.json`.
...
Your MCPs are available as tools in Renx, alongside built-in tools. So you can manage them through the Renx config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Renx to your Sentry account.
