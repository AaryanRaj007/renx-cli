...
As habilidades do agente permitem que o renx descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O renx pesquisa nesses locais:

- Configuração do projeto: `.renx/skills/<name>/SKILL.md`
- Configuração global: `~/.config/renx/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o renx sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.renx/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/renx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.renx/skills/git-release/SKILL.md` assim:
...
O renx lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `renx.json`:
...
**Para agentes embutidos** (em `renx.json`):
