...
Com o renx, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o renx usa nosso próprio tema `renx`.
...
O renx vem com vários temas embutidos.
...
- Querem que o renx corresponda à aparência do seu terminal
...
O renx suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/renx/themes/*.json` ou `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.renx/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.renx/themes/*.json`
