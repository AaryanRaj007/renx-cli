---
title: Servidor
description: Interaja com o servidor renx via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `renx serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente renx pode usar.
...
```bash
renx serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `renx.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
renx serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `RENX_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `renx`, ou defina `RENX_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `renx serve` quanto ao `renx web`.

```bash
RENX_SERVER_PASSWORD=your-password renx serve
```
...
Quando você executa `renx`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor renx para interagir com o renx programaticamente.
:::

Essa arquitetura permite que o renx suporte múltiplos clientes e permite que você interaja com o renx programaticamente.

Você pode executar `renx serve` para iniciar um servidor autônomo. Se você tiver o TUI do renx em execução, `renx serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do renx [IDE](/docs/ide).
...
O servidor renx expõe as seguintes APIs.
