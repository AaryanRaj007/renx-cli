---
title: Plugins
description: Escreva seus próprios plugins para estender o renx.
---

Plugins permitem que você estenda o renx conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do renx.
...
- `.renx/plugins/` - Plugins em nível de projeto
- `~/.config/renx/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/renx/node_modules/`.
...
1. Configuração global (`~/.config/renx/renx.json`)
2. Configuração do projeto (`renx.json`)
3. Diretório de plugins global (`~/.config/renx/plugins/`)
4. Diretório de plugins do projeto (`.renx/plugins/`)
...
O renx executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do renx para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o renx.
...
:::note
Se você estiver usando o aplicativo desktop renx, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o renx de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao renx:
...
O helper `tool` cria uma ferramenta personalizada que o renx pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o renx junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://renx.ai/docs/sdk) para detalhes.
