...
For at fejlfinde problemer med Renx, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\renx\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `renx --log-level DEBUG`.

---

## Lagring

renx gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/renx/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\renx`
...
Renx Desktop kører en lokal Renx-server (`renx-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Renx`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/renx/renx.jsonc` (eller `~/.config/renx/renx.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/renx/renx.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\renx\renx.jsonc`
...
Renx kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/renx/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\renx\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Renx kan genopbygge den.

1. Afslut Renx Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/renx`
- **Linux**: slet `~/.cache/renx` (eller kør `rm -rf ~/.cache/renx`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\renx`

3. Start Renx Desktop på ny.

---

### Løs problemer med serverforbindelse

Renx Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `renx.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `RENX_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `RENX_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Renx Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Renx.

---

### Meddelelser vises ikke

Renx Desktop viser kun systemvarsler når:

- varsler er aktiveret for Renx i dine OS-indstillinger, og
...
1. Afslut Renx Desktop.
2. Find og slet disse filer (de findes i Renx Desktop-appens datamappe):

- `renx.settings.dat` (skrivebordsstandardserver URL)
- `renx.global.dat` og `renx.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Renx:
...
For at finde ud af hvilke modeller du har adgang til, kør `renx models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\renx`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. renx installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\renx`

2. Start renx på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge renx til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
renx vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
