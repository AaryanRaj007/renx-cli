...
Med Renx kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Renx vårt eget `renx`-tema.
...
Renx kommer med flere innebygde temaer.
...
- Vil at Renx skal matche terminalens utseende
...
Renx støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/renx/themes/*.json` eller `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.renx/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.renx/themes/*.json`
