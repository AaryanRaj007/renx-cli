...
Her er alle de innebygde verktøyene som er tilgængelige i Renx.
...
:::note
Dette verktøyet er bare tilgængelig når `RENX_EXPERIMENTAL_LSP_TOOL=true` (eller `RENX_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Renx-leverandøren eller når miljøvariabelen `RENX_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
