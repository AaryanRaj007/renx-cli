---
title: Server
description: Interager med renx-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `renx serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en renx-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `renx.local` |
...
Indstil `RENX_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `renx`, eller indstil `RENX_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `renx serve` og `renx web`.
...
Når du kører `renx` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug renx-serveren til at interagere med renx programmatisk.
:::

Denne arkitektur lader renx understøtte flere klienter og lader dig interagere med renx programmatisk.

Du kan køre `renx serve` for at starte en selvstændig server. Hvis du har
renx TUI kørende, vil `renx serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Renx [IDE](/docs/ide) plugins.
...
Renx-serveren viser følgende API'er.
