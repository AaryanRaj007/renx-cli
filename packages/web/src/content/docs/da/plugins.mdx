---
title: Plugins
description: Skriv dine egne plugins for at udvide Renx.
---

Plugins giver dig mulighed for at udvide Renx ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Renxs standardadfærd.
...
- `.renx/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/renx/node_modules/`.
...
1. Global konfiguration (`~/.config/renx/renx.json`)
2. Projektkonfiguration (`renx.json`)
3. Global plugin-mappe (`~/.config/renx/plugins/`)
4. Projekt plugin bibliotek (`.renx/plugins/`)
...
Renx kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En renx SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide renx.
...
:::note
Hvis du bruger Renx desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå renx i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til renx:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som renx kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for renx sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://renx.ai/docs/sdk) for detaljer.
