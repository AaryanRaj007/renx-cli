---
title: Plugins
description: Écrivez vos propres plugins pour étendre Renx.
---

Les plugins vous permettent d'étendre Renx en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Renx.
...
- `.renx/plugins/` - Plugins au niveau du projet
- `~/.config/renx/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/renx/node_modules/`.
...
1. Configuration globale (`~/.config/renx/renx.json`)
2. Configuration du projet (`renx.json`)
3. Répertoire global des plugins (`~/.config/renx/plugins/`)
4. Répertoire des plugins du projet (`.renx/plugins/`)
...
Renx exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK renx pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre renx.
...
:::note
Si vous utilisez l'application de bureau Renx, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez renx de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à renx :
...
L'assistant `tool` crée un outil personnalisé que renx peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour renx aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://renx.ai/docs/sdk) pour plus de détails.
