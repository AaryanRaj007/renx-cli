...
Vous pouvez ajouter des outils externes à Renx à l'aide du _Model Context Protocol_ ou MCP. Renx prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Renx Config](https://renx.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/renx`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Renx gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Renx :
...
Si le serveur nécessite une authentification, Renx vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `renx mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Renx stockera les jetons en toute sécurité dans `~/.local/share/renx/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Renx, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Renx comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Renx à votre compte Sentry.
