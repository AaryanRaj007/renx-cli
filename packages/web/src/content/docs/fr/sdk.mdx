---
title: SDK
description: Client JS de type sécurisé pour le serveur renx.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK renx JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler renx par programme.
...
Créez une instance de renx :

```javascript
import { createRenx } from "@renx-ai/sdk"

const { client } = await createRenx()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `renx.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${renx.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de renx, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@renx-ai/sdk"
```
