...
Avec Renx, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Renx utilise notre propre thème `renx`.
...
Renx est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Renx corresponde à l'apparence de leur terminal
...
Renx prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/renx/themes/*.json` ou `$XDG_CONFIG_HOME/renx/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.renx/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.renx/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/renx/themes
vim ~/.config/renx/themes/my-theme.json
```
...
{
  "$schema": "https://renx.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
