---
title: Serveur
description: Interagissez avec le serveur renx via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `renx serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client renx peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `renx.local` |
...
Définissez `RENX_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `renx`, ou définissez `RENX_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `renx serve` et à `renx web`.
...
Lorsque vous exécutez `renx`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur renx pour interagir avec renx par programmation.
:::

Cette architecture permet à renx de prendre en charge plusieurs clients et vous permet d'interagir avec renx par programme.

Vous pouvez exécuter `renx serve` pour démarrer un serveur autonome. Si vous avez le
renx TUI en cours d'exécution, `renx serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Renx [IDE](/docs/ide).
...
Le serveur renx expose les API suivantes.
