...
Il existe deux types d'agents dans Renx ; agents primaires et sous-agents.
...
Renx est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Renx est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Renx est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `renx.json` :
...
- Global : `~/.config/renx/agents/`
- Par projet : `.renx/agents/`

```markdown title="~/.config/renx/agents/review.md"
---
...
Si aucune température n'est spécifiée, Renx utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Renx et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Renx utilise le format `provider/model-id`. Par exemple, si vous utilisez [Renx Zen](/docs/zen), vous utiliserez `renx/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `renx models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/renx).
:::
...
```markdown title="~/.config/renx/agents/docs-writer.md"
---
...
```markdown title="~/.config/renx/agents/security-auditor.md"
---
