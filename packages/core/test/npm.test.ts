...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@renx/acme")).toBe("@renx/acme")
    expect(Npm.sanitize("@renx/acme@1.0.0")).toBe("@renx/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/renx/acme.git"
    const expected = win ? "acme@git+https_//github.com/renx/acme.git" : spec
