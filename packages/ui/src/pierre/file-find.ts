...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("renx-find")
  api.delete("renx-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("renx-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("renx-find", new Highlight(...rest))
    return true
  }
