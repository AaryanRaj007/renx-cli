...
import "./styles.css"
import { useTheme } from "@renx/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.RENX_CHANNEL === "prod" &&
...
const deepLinkEvent = "renx:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__RENX__ ??= {}
  const pending = window.__RENX__.deepLinks ?? []
  window.__RENX__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("renx.global.dat").getItem("language")
