...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.renx.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `renx.settings.dat` is special: it maps to the `renx.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"renx.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // renx.settings.dat → the electron settings store ("renx.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "renx.global.dat").
  const storeName = filename === "renx.settings.dat" ? "renx.settings" : filename
