...
const APP_IDS: Record<string, string> = {
  dev: "ai.renx.desktop.dev",
  beta: "ai.renx.desktop.beta",
  prod: "ai.renx.desktop",
}
const TEST_ONBOARDING = process.env.RENX_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.RENX_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.renx.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `renx-onboarding-${randomUUID()}`)
    rmSync(root, { recursive: true, force: true })
    ;["data", "config", "cache", "state", "desktop", "session"].forEach((dir) =>
      mkdirSync(join(root, dir), { recursive: true }),
    )
    process.env.RENX_DB = ":memory:"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Renx Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("renx://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("renx")
  registerRendererProtocol()
  setDockIcon()
  setupAutoUpdater()

  const needsMigration = ((): boolean => {
    if (process.env.RENX_DB === ":memory:") return false

    const xdg = process.env.XDG_DATA_HOME
    const base = xdg && xdg.length > 0 ? xdg : join(homedir(), ".local", "share")
    return !existsSync(join(base, "renx", "renx.db"))
  })()
  let overlay: BrowserWindow | null = null

  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.RENX_PORT
    if (fromEnv) {
