...
import { createRenx } from "@renx-ai/sdk"
...
`)
  ...
  const renx = await createRenx({ port: 0 })
  ...
  try {
    ...
    const session = await renx.client.session.create()
    const result = await renx.client.session
      .prompt({
