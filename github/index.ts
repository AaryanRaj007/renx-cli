...
import { createRenxClient } from "@renx-ai/sdk"
...
const { client, server } = createRenx()
...
try {
  ...
  console.log("renx session", session.id)
  if (shareId) {
...
function createRenx() {
  ...
  const proc = spawn(`renx`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createRenxClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/renx|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/renx` or `/oc`")
  }
}
...
async function assertRenxConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to renx server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.renx.ai" : "https://renx.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.renx.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("renx-github-action")
    response = await fetch("https://api.renx.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/renx" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/renx") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to renx...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "renx-agent[bot]"`
  await $`git config --global user.email "renx-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `renx/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/renx-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[renx session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
